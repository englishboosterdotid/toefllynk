...
import { Package, CheckCircle2, Shield, ArrowLeft, Star, Zap, BookOpen, ExternalLink, Users, BadgeCheck, Clock } from "lucide-react";
...
export default async function MicrositeProductPage({
  ...
  const micrositeOwner = await prisma.user.findUnique({
    where: { username },
    include: {
  ...
  if (!micrositeOwner) return notFound();
  ...
  const product = await prisma.product.findUnique({
    ...
    include: {
      ...
      affiliateEnrollments: ref ? {
        ...
        select: { affiliateUserId: true, commissionPercent: true },
        take: 1,
      } : false,
    },
  });
  ...
  const isAffiliateProduct = product.userId !== micrositeOwner.id;
  const isValidAffiliate = isAffiliateProduct && ref && product.affiliateEnrollments && product.affiliateEnrollments.length > 0;
  ...
  const checkoutUrl = isValidAffiliate
    ? `/checkout/${product.id}?ref=${ref}`
    : `/checkout/${product.id}`;
          ...
          <div className="flex items-start gap-5 mb-6">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {product.user.id === micrositeOwner.id && (
              ...
              {isValidAffiliate && (
                <p className="text-emerald-400 text-sm mt-1">
                  Rekomendasi dari @{username} • {product.affiliateEnrollments?.[0]?.commissionPercent}% komisi
                </p>
              )}
            </div>
          </div>
